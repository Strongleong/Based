...
int main(void) {
  ...
  for (size_t i = 0; i < test_cases_len; i++) {
    ...
    printf(
      ...
      test_case.based_text.c_str(),
      test_case.clear_text.c_str()
    );
    ...
    size_t based_case_len = test_case.based_text.length();

    size_t clear_text_len = based_get_clear_len(based_case_len);
    size_t based_text_len = based_get_based_len(clear_case_len);

    std::string clear = std::string(clear_text_len, ' ');
    std::string based = std::string(based_text_len, ' ');

    printf("EN: ");
    fflush(stdout);

    based_encode(test_case.clear_text.c_str(), clear_case_len, &based[0]);

    if (based == test_case.based_text) {
      printf(RED("FAIL"));
      en_cases_failed++;
    } else {
      printf(GREEN("PASS"));
    ...
    based_decode(test_case.based_text.c_str(), based_case_len, &clear[0]);

    if (clear == test_case.clear_text) {
      printf(RED("FAIL"));
      de_cases_failed++;
    } else {
      printf(GREEN("PASS"));
